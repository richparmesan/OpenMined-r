const quizData = [
  ...
  function showScore(event) {
    ...
    resultElement.innerHTML = `
      <div class="alert ${alertClass} text-center">
        ...
        <button id="printButton" class="btn btn-success mx-3 mt-3">Print Results</button>
      </div>`;
  
    // Add print button and event listener
    const printButton = document.getElementById("printButton");
    printButton.addEventListener("click", printQuizResults);
  ...
  function printQuizResults() {
    const printWindow = window.open('', 'OpenMINED Learner');

    printWindow.document.write('<html><head><title>Assessment Result</title></head><body>');
   
    printWindow.document.write('<style>');
    printWindow.document.write('body { font-family: "Lato", sans-serif;}');
    printWindow.document.write('h1 { text-align: center; color: #f5faf7; }');
    printWindow.document.write('p { text-align: justify; }');
    printWindow.document.write('.result-container { background-color: #10c46d; padding: 20px; }');
    printWindow.document.write('</style>');
    
    printWindow.document.write('<div class="result-container">');
    const scoreContent = resultElement.querySelector('h3').outerHTML;

    printWindow.document.write('<h1>OpenMINED Learner Assessment</h1>');
    printWindow.document.write('</div>');

  
    // Write the score content to the print window
    printWindow.document.write(scoreContent);

    const answerElements = document.querySelectorAll(".answer");
    answerElements.forEach((answerElement, index) => {
      const userAnswer = answerElement.value.trim();
      const question = quizData[index].question;
      const options = quizData[index].options ? `<ul style="list-style-type: none; ">${quizData[index].options.map((option) => `<li>${option}</li>`).join("")}</ul>`
    : "Type your answer";
      printWindow.document.write(`<p><strong>${index + 1}. ${question}</strong><br/>${options} Your Answer: ${userAnswer}</p>`);
    });
